...
// Extra robust ID cleaning: if the query contains a number, also try searching specifically for that number
$numeric_part = preg_replace('/[^0-9]/', '', $clean_query);

try {
    if ($type === 'patients') {
        // Search by name, email, phone, or ID (both partial and exact)
        $where = "WHERE (p.first_name LIKE :s OR p.last_name LIKE :s OR p.email LIKE :s OR p.phone LIKE :s OR CAST(p.patient_id AS CHAR) LIKE :s)";
        $params = [':s' => $search];
        
        // If there's a numeric part, prioritize it in the search or allow exact match
        if (!empty($numeric_part)) {
            $where = "WHERE (p.first_name LIKE :s OR p.last_name LIKE :s OR p.email LIKE :s OR p.phone LIKE :s OR CAST(p.patient_id AS CHAR) LIKE :s OR p.patient_id = :id_exact)";
        ...
        $where = "WHERE (first_name LIKE :s OR last_name LIKE :s OR specialization LIKE :s OR email LIKE :s OR CAST(doctor_id AS CHAR) LIKE :s)";
        $params = [':s' => $search];
        if (!empty($numeric_part)) {
            $where = "WHERE (first_name LIKE :s OR last_name LIKE :s OR specialization LIKE :s OR email LIKE :s OR CAST(doctor_id AS CHAR) LIKE :s OR doctor_id = :id_exact)";
            $params[':id_exact'] = $numeric_part;
        }
        ...
        $where = "WHERE (p.first_name LIKE :s OR p.last_name LIKE :s OR p.email LIKE :s OR p.phone LIKE :s OR CAST(a.appointment_id AS CHAR) LIKE :s)";
        $params = [':s' => $search];
        if (!empty($numeric_part)) {
            $where = "WHERE (p.first_name LIKE :s OR p.last_name LIKE :s OR p.email LIKE :s OR p.phone LIKE :s OR CAST(a.appointment_id AS CHAR) LIKE :s OR a.appointment_id = :id_exact)";
            $params[':id_exact'] = $numeric_part;
        }
